...
def tempCheck(tempVal):
	...
	if(tempVal < 120 and tempVal>60):
	#This check (above) works around a glitch where the sensor reads 0 then 700 on loop for a while which causes hundreds of emails to be sent in minutes
        	if(alreadySentT == 0 and count > 2):
#            		print('Your temp is out of Range! \n')
            		sendTempMail(tempVal)
            		alreadySentT = 1
    if(tempVal<(maxTemp-4) and tempVal>(minTemp+4) and alreadySentT == 1):
        alreadySentT = 0
...
#maybe add a time stamp
    ...
    msg['From'] = fromAddress
    msg['To'] = toAddress
    msg['Subject'] = "GREENHOUSE ALERT: Water level out of Range"
...
#            print ('stripped Pval is ' + val2)
            ...
            pCount += 1
	    if pCount == 50:
	    	reading = (str(datetime.date.today()),time.strftime("%H:%M:%S"),phVal,'PH')
            	insert_reading(conn,reading)
		pCount = 0
...
#            print ('stripped Dval is ' + val2)
	    ...
	    dCount += 1
	    if dCount == 50:
            	reading = (str(datetime.date.today()),time.strftime("%H:%M:%S"),doVal,'DO')
            	insert_reading(conn,reading)
		dCount = 0
...
#            print ('stripped Eval is ' + val2)
	    ...
	    eCount += 1
	    if eCount == 50:
            	reading = (str(datetime.date.today()),time.strftime("%H:%M:%S"),ECVal,'EC')
            	insert_reading(conn,reading)
		eCount = 0
...
#	5/14/17 - turned on 2nd arduino
#	6/4/17 - fixed timing on database uploads
#	9/1/17 - commeted out print statements
#	9/1/17 - added a workaround for temp email glitch
